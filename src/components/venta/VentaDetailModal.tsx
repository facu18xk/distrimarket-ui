...
import type { VentaItem } from '../../types/distrimarket';
...
export const VentaDetailModal: React.FC<VentaDetailModalProps> = ({ isOpen, onClose, venta }) => {
  ...
  return (
    <ModalDialog
      isOpen={isOpen}
      onClose={onClose}
      title={`Detalle de Venta ${venta.numeroComprobante}`}
      subtitle={`Referencia: ${venta.referencia} • Tipo: ${venta.tipoVenta}`}
      maxWidth="lg"
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 text-xs">
          <div>
            ...
            <strong className="text-slate-800">{venta.cliente}</strong>
          </div>
          <div>
            <span className="text-slate-400 block">Fecha:</span>
            <strong className="text-slate-800">{venta.fecha}</strong>
          </div>
          <div>
            <span className="text-slate-400 block">Canal:</span>
            <strong className="text-slate-800">{venta.tipoVenta}</strong>
          </div>
          <div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
              Productos Consumidos ({detalles.length})
            </h4>
        ...
        <div className="flex items-center justify-between p-3 bg-slate-900 text-white rounded-xl">
          <span className="text-xs font-semibold uppercase tracking-wider">Total Cobrado (IVA Incluido):</span>
